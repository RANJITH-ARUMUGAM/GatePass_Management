...
// import { Eye, EyeOff, Lock, User, Shield, CheckCircle, AlertCircle, X } from 'lucide-react';
...
//   const mail = ReactSession.get('username');
//   const oldpassword = ReactSession.get('password');
//   const userPhoto = ReactSession.get('profileimage');
...
//     alert('Navigation would occur in real implementation');
//   };
...
//   // Common passwords and patterns to avoid (Google-style validation)
...
//     const criteria = {
//       length: password.length >= 12, // Google recommends 12+ characters
...
//       setError('Password must be at least 9 characters, contain uppercase, lowercase, number, and special character');
//       return;
//     }
...
//     try {
//       const response = await axios.put(`${SERVER_PORT}/change_password/${mail}`, { newPassword });
...
//   return (
//     <div
//       className="flex flex-col md:flex-row bg-gradient-to-br from-indigo-50 to-blue-100 shadow-xl rounded-3xl overflow-hidden max-w-5xl mx-auto my-2"
//       style={{ minHeight: 140 }}
//     >
...
//             src={userPhoto}
//             alt="Profile"
//             className="w-196 h-196 rounded-full object-cover ring-4 ring-indigo-200 shadow-lg mb-1"
//           />
//         ) : (
...
//               className="w-full pl-3 pr-10 py-1.5 border border-indigo-200 rounded-lg bg-indigo-50 focus:ring-2 focus:ring-indigo-400 outline-none transition text-sm"
...
//               <span className={`font-semibold ${getPasswordStrengthInfo().textColor}`}>
//                 {getPasswordStrengthInfo().level}
//               </span>
//             </div>
//             <div className="w-full bg-indigo-100 rounded-full h-1">
//               <div
//                 className={`h-1 rounded-full ${getPasswordStrengthInfo().color}`}
...
//         <button
//           type="submit"
//           className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-2 rounded-lg font-semibold shadow mt-2 transition text-sm"
//         >
...
//   useEffect(() => {
//     // Convert the userPhoto buffer to a data URL when the component mounts or userPhoto changes
//     if (userPhoto) {
...
//     // Validate current password on the server for better security
//     try {
...
const ChangePassword = ({ setTitle }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const checkResponse = await axios.post(`${SERVER_URL}/validate_password`, {
        email: mail,
        password: currentPassword
      });
      ...
      const changeResponse = await axios.put(`${SERVER_URL}/change_password/${mail}`, { newPassword });
