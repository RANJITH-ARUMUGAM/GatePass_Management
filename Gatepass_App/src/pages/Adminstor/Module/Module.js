...
function Module({ setTitle }) {
    ...
    useEffect(() => {
        ...
        axios.get(`${SERVER_PORT}/moduleload`)
            .then(res => {
                ...
                const InactiveRecords = res.data.filter(record => record.Module_Valid_Converted == 'Inactive');
    ...
    const colDropdownfilterName = (e) => {
        ...
        const sortedValues = uniqueValues.sort((a, b) => {
            const aIsSelected = SelectedItemsName.includes(a.Module_Name);
            const bIsSelected = SelectedItemsName.includes(b.Module_Name);
                ...
                <Row className="align-items-center">
                    <Col xs={12}>
                        <div className="d-flex justify-content-between align-items-center flex-nowrap" style={{ gap: '16px', flexWrap: 'nowrap' }}>
                            <div className="d-flex align-items-center flex-nowrap" style={{ gap: '4px', flexWrap: 'nowrap' }}>
                                <Link className="custom-Nav-header" to='/Home'>Administration</Link>
                                ...
                                <span className="custom-Nav-Sub-header">Module</span>
                            </div>
                            <Button type="button" className="btn-custom1 btn-sm" variant="success">
                                <Link to="/addmodule" style={{ color: 'white', textDecoration: 'none', display: 'flex', alignItems: 'center' }}>
                                    <OverlayTrigger
                                        placement="top"
                                ...
                                <tr>
                                    ...
                                    <th >
                                        <div className="header-cell d-flex">
                                            <span className="clickable" onClick={(e) => sortResult1('Module_Code', true)}>
                                                Module Code<span>{setSortasc.sortdirection1 === "DOWN" ? '▼' : '▲'}</span>
                                            </span>
                                        </div>
                                    </th>
                                    <th >
                                        <div className="header-cell d-flex">
                                            <span className="clickable" onClick={(e) => sortResult2('Module_Name', true)}>
                                                Module Name<span>{setSortasc.sortdirection2 === "DOWN" ? '▼' : '▲'}</span>
                                            </span>
                                            <span className="filter-container d-flex">
                                                ...
                                                <div className={FilteronClickLightName ? 'led-yellow' : ''} style={{ marginLeft: '8px' }}></div>
                                            </span>
                                        </div>
                                    </th>
                                    <th >
                                        <div className="header-cell d-flex">
                                            <span className="clickable" onClick={(e) => sortResult3('Module_Valid_Converted', true)}>
                            ...
                            <tbody>
                                {currentItems.map((mod, index) => (
                                    <tr key={mod.Module_ID}>
                                        ...
                                        <td style={{ width: '100px' }} >{mod.Module_Application}</td>
                                        <td style={{ width: '160px' }} >{mod.Module_Code}</td>
                                        <td style={{ width: '650px' }} >{mod.Module_Name}</td>
                                        <td style={{ width: '120px' }} >{changeValid(mod.Module_Valid_Converted.toString())}</td>
                                        <td style={{ width: '100px' }} >
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', justifyContent: 'center' }}>
                                                ...
                                                <Button className="p-1" style={{ minWidth: 0 }}>
                                                    ...
                                                    >
                                                        <Link to={`/editmodule/${mod.Module_ID}`} style={{ color: 'green', display: 'flex', alignItems: 'center' }}>
                                                ...
                                                {/* Delete Button */}
                                                <Button className="p-1" style={{ minWidth: 0 }} onClick={() => deleteClick(mod.Module_ID)}>
                                                    <OverlayTrigger
                                                        placement="top"
